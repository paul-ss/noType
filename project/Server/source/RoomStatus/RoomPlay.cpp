...
ExpectedRoom<size_t> RoomPlay::validateWrittenText(std::shared_ptr<Room> room,
  ...
  if (room->_players[clientUUID]._state != PLAYER_PLAY) {
    return RoomError("Sorry, you can't enter the text, because you have already finished! Congratulations!");
  }

  size_t rightWritten = compareText(room->_text, recvText, room->_players[clientUUID]._textPosition);
  room->_players[clientUUID]._textPosition += rightWritten;

  if (room->_players[clientUUID]._textPosition >= room->_text.size()) {
    if (room->_numberOfFinishers == 0) {
      room->_players[clientUUID]._state = PLAYER_WIN;
    } else {
      room->_players[clientUUID]._state = PLAYER_FINISH;
    }
